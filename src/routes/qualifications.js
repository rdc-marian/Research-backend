...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const items = await Qualification.find(query)
...
router.post(
  ...
  upload.single("file"),
  asyncHandler(async (req, res) => {
    const { scholarId, degree, subject, institution, university, yearOfPassing, percentage, status } = req.body;

    if (!scholarId || !degree || !subject || !institution || !university || !yearOfPassing) {
      res.status(400).json({ message: "scholarId, degree, subject, institution, university, and yearOfPassing are required" });
      return;
    }
    ...
    if (req.file) {
      ...
      const result = await uploadBuffer(req.file.buffer, {
        folder: "portfolio-qualifications",
    ...
    const item = await Qualification.create({
      scholar: scholarId,
      degree,
      subject,
      institution,
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Always reset status to Pending on edit so guide re-verifies
    update.verificationStatus = "Pending";
    update.verifiedBy = undefined;
    update.verifiedAt = undefined;

    const item = await Qualification.findByIdAndUpdate(req.params.id, update, {
      new: true,
      runValidators: true,
    }).populate("scholar", "name email department guide");
