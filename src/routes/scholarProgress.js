...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const items = await ScholarProgress.find(query)
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.file) {
      ...
      const result = await uploadBuffer(req.file.buffer, {
        folder: "portfolio-scholarProgress",
    ...
    const item = await ScholarProgress.create(data);
...
router.get(
  "/:id",
  asyncHandler(async (req, res) => {
    const item = await ScholarProgress.findById(req.params.id)
      .populate("scholar", "name email researchCenter guide")
      .populate("verifiedBy", "name email");
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const item = await ScholarProgress.findByIdAndUpdate(req.params.id, update, {
      new: true,
      runValidators: true,
    }).populate("scholar", "name email researchCenter guide");
...
router.delete(
  "/:id",
  asyncHandler(async (req, res) => {
    const item = await ScholarProgress.findByIdAndDelete(req.params.id);
    if (!item) {
      res.status(404).json({ message: "Item not found" });
      return;
    }
