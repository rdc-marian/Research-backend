...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const items = await ResourcePerson.find(query)
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.file) {
      ...
      const result = await uploadBuffer(req.file.buffer, {
        folder: "portfolio-resourcePerson",
    ...
    const item = await ResourcePerson.create(data);
...
router.get(
  "/:id",
  asyncHandler(async (req, res) => {
    const item = await ResourcePerson.findById(req.params.id)
      .populate("scholar", "name email researchCenter guide")
      .populate("verifiedBy", "name email");
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const item = await ResourcePerson.findByIdAndUpdate(req.params.id, update, {
      new: true,
      runValidators: true,
    }).populate("scholar", "name email researchCenter guide");
...
router.delete(
  "/:id",
  asyncHandler(async (req, res) => {
    const item = await ResourcePerson.findByIdAndDelete(req.params.id);
    if (!item) {
      res.status(404).json({ message: "Item not found" });
      return;
    }
