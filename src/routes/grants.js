...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const items = await ResearchGrant.find(query)
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const item = await ResearchGrant.create(data);
...
router.get(
  "/:id",
  asyncHandler(async (req, res) => {
    const item = await ResearchGrant.findById(req.params.id)
      .populate("scholar", "name email researchCenter guide")
      .populate("verifiedBy", "name email");
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const item = await ResearchGrant.findByIdAndUpdate(req.params.id, update, {
      new: true,
      runValidators: true,
    }).populate("scholar", "name email researchCenter guide");
...
router.delete(
  "/:id",
  asyncHandler(async (req, res) => {
    const item = await ResearchGrant.findByIdAndDelete(req.params.id);
    if (!item) {
      res.status(404).json({ message: "Item not found" });
      return;
    }
