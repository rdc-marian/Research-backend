...
router.post(
  ...
  upload.single("file"),
  asyncHandler(async (req, res) => {
    const { scholarId, professionalBody, membershipNumber, membershipType, startDate, expiryDate } = req.body;

    if (!scholarId || !professionalBody || !membershipNumber || !startDate) {
      res.status(400).json({ message: "scholarId, professionalBody, membershipNumber, and startDate are required" });
      return;
    }
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const item = await Membership.findByIdAndUpdate(req.params.id, update, {
      new: true,
      runValidators: true,
    }).populate("scholar", "name email department guide");
