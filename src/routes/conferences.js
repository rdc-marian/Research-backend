...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const items = await Conference.find(query)
...
router.post(
  ...
  upload.single("file"),
  asyncHandler(async (req, res) => {
    const { scholarId, title, paperTitle, presentationType, organizer, venue, startDate, endDate, proceedingsDetails } = req.body;
    ...
    const item = await Conference.create({
      ...
      title,
      paperTitle,
      presentationType: presentationType || "Attended Only",
      organizer,
      venue,
...
router.get(
  "/:id",
  asyncHandler(async (req, res) => {
    const item = await Conference.findById(req.params.id)
      .populate("scholar", "name email department guide")
      .populate("verifiedBy", "name email");
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const item = await Conference.findByIdAndUpdate(req.params.id, update, {
      new: true,
      runValidators: true,
    }).populate("scholar", "name email department guide");
